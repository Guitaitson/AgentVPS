...
@pytest.mark.asyncio
async def test_telegram_progress_session_prefers_custom_labels(monkeypatch):
    monkeypatch.setattr(bot_module, "PROGRESS_MESSAGE_THRESHOLD_SECONDS", 0.0)
    monkeypatch.setattr(bot_module, "TYPING_INTERVAL_SECONDS", 999.0)
    ...
    await session.start()
    await asyncio.sleep(0)
    await session.on_progress(
        "external_call",
        {"server": "codex_operator", "label": "Operador Codex em execucao ha 30s. Status: normal."},
    )
    await asyncio.sleep(0)

    assert update.message.sent[0].edits[-1] == "Operador Codex em execucao ha 30s. Status: normal."
