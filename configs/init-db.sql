...
CREATE INDEX idx_voice_jobs_status ON voice_ingestion_jobs(status, started_at DESC);
CREATE INDEX idx_voice_files_job_status ON voice_audio_files(job_id, status, updated_at DESC);
CREATE INDEX idx_voice_context_commit_status ON voice_context_items(commit_status, created_at DESC);
CREATE INDEX idx_voice_context_proposal ON voice_context_items(proposal_id);
