...
- `core/integrations/consumer_sync.py` faz machine-pull do credential bundle do consumidor `agentvps`, anuncia `client_capabilities` no request, persiste `current_release_id`, `current_bundle_hash`, `current_bundle`, `contract`, `client_adaptation` e `rollout_status` em `data/consumer-sync/agentvps.json` e fornece as credenciais dinamicamente para FleetIntel/BrazilCNPJ
...
- `core/integrations/consumer_contract_validation.py` executa a validacao canary da release publicada como `sync -> checks -> validation-report -> sync`; so considera a canary concluida quando o segundo sync retorna `rollout_status.status=canary_passable`
